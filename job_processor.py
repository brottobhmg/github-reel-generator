...
class JobProcessor:
    ...
    def __init__(
        ...
        Args:
            ...
            client: Optional pre-configured HTTP client (useful for tests).
        """
        self.base_url = (base_url or settings.api_url).rstrip("/")
        self._client = client or httpx.Client(timeout=NOTIFY_TIMEOUT)
    ...
    def download_video(self, job_id: int, dest_dir: Path) -> Path:
        ...
        with self._client.stream("GET", url, timeout=VIDEO_TIMEOUT) as response:
    ...
    def download_json(self, job_id: int) -> dict:
        ...
        response = self._client.get(url, timeout=JSON_TIMEOUT)
        response.raise_for_status()
        data = response.json()
    ...
    def notify_completed(self, job_id: int) -> None:
        ...
        response = self._client.post(url, timeout=NOTIFY_TIMEOUT)
        response.raise_for_status()
